...
def process_data(file, is_en):
    # 选择前两千行训练
    # 训练完成后可以删去行数限制，并修改相关代码以加载已存在的模型
    # 然后可对所有句子进行测试
    df = pd.read_csv(file, delimiter='\t', header=None, names=["english", "chinese"], nrows=2000)
    datas = []
    for i, row in df.iterrows():
        if is_en:
            line = row["english"].strip()
            datas.append(["BOS"] + nltk.word_tokenize(line.lower()) + ["EOS"])
        else:
            line = row["chinese"].strip()
            datas.append(["BOS"] + list(jieba.cut(line, cut_all=False)) + ["EOS"])
    return datas


# en_path = "D:/JetBrains/Python Files/seq2seq/data/news-commentary-v12.zh-en_en.txt"
# cn_path = "D:/JetBrains/Python Files/seq2seq/data/news-commentary-v12.zh-en_zh.txt"
# 更换数据集
en_path = "D:/JetBrains/Python Files/seq2seq/data/cmn_zhsim.txt"
cn_path = "D:/JetBrains/Python Files/seq2seq/data/cmn_zhsim.txt"
en = process_data(en_path, is_en=True)
cn = process_data(cn_path, is_en=False)
...
#                   dec_hidden_size=dec_hidden_size,
#                   dropout=dropout)
# 2. 或加载已存在的encoder和decoder
encoder = torch.load("D:/JetBrains/Python Files/seq2seq/models/encoder_test3.mdl")
decoder = torch.load("D:/JetBrains/Python Files/seq2seq/models/decoder_test3.mdl")
...
def train(model, data, epoches):
    ...
    for epoch in range(epoches):
        ...
        for it, (batch_x, batch_x_len, batch_y, batch_y_len) in enumerate(data):
            ...
            if (it % 50 == 0):
                print("Epoch {} / {}, Iteration: {}, Train Loss: {}".format(epoch, epoches - 1, it, loss.item()))
        print("Epoch {} / {}, Train Loss: {}".format(epoch, epoches - 1, total_loss / total_words))
        if (epoch != 0 and epoch % 100 == 0):
            avg_loss = test(model, test_datasets)
            print("Test Loss:", avg_loss)
            return avg_loss


MAX_LENGTH = 10
nxt = [0 for i in range(MAX_LENGTH+10)]
...
def getTrans(trans):
    getnxt(trans)
    transLen = len(trans)
    L = transLen - nxt[transLen]
    return trans[:L]


def en2cn_translate_id(sentence_id):
    # 英文翻译成中文
    ...
    print("chinese:", cn_sentence)
    print("predict:", " ".join(trans))


if __name__ == "__main__":
    # 训练代码，不需要时注释掉
    # while True:
    #     avg_loss = train(model, datasets, epoches=101)
    #     # 存储训练后的encoder和decoder
    #     torch.save(encoder, "D:/JetBrains/Python Files/seq2seq/models/encoder_test3.mdl")
    #     torch.save(decoder, "D:/JetBrains/Python Files/seq2seq/models/decoder_test3.mdl")
    #     if avg_loss > 0.5:
    #         continue
    #     else:
    #         break
    # 测试
    while True:
        sentence_id = input("请输入一个句子id: ")
        en2cn_translate_id(sentence_id)
